    ...
    public static class ObjectSerializer
    {
        ...
        public static string SerializeChunk(Chunk chunk)
        {
            ...
            for (int i = 0; i < 64; i++)
            {
                for (int j = 0; j < 64; j++)
                {
                    Background background = chunk.backgrounds[i, j];
                    if (background != null)
                    {
                        // Serialize each tile's properties
                        sb.AppendLine($"{background.x},{background.y},{(int)background.type}");
                    }
                    else
                    {
                        sb.AppendLine("null");
        ...
        public static Chunk DeserializeChunk(string base64SerializedData)
        {
            ...
            // Convert the decompressed byte array back to a string
            string serializedData = Encoding.UTF8.GetString(Convert.FromBase64String(base64SerializedData));
            ...
            for (int i = 0; i < 64; i++)
            {
                for (int j = 0; j < 64; j++)
                {
                    string backgroundData = lines[lineIndex++];
                    if (backgroundData != "null")
                    {
                        var parts = backgroundData.Split(',');
                        Background background = new Background
                        {
                            x = int.Parse(parts[0]),
                            y = int.Parse(parts[1]),
                            type = (Tile.TileType)int.Parse(parts[2])
                        };
                        chunk.backgrounds[i, j] = background;
                    }
                }
            }
